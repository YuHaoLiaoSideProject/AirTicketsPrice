...
| E2 | 訂閱失敗（subscribe 拋錯 / /subscribe 非 2xx） | 狀態「訂閱失敗，請稍後重試」；subscribe 拋 `AbortError`（瀏覽器連不上推播服務：VPN／公司網路／擋廣告擴充常見）→「通知服務連線失敗，請確認網路後重試（VPN／公司網路／擋廣告擴充可能阻擋推播服務）」；Brave（`navigator.brave`）→ 專屬提示「點獅子圖示關閉本站 Shields」；按鈕可重試；圖表/航線/離線完全不受影響 | @error p0 / F-09, E2E-16 |
